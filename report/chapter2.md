...
<table>
  <thead>
    <tr>
      ...
      <th colspan="2">Carlos Mendoza<br>Administrador</th>
      <th colspan="2">Luis Fernández<br>Conductor</th>
    </tr>
    <tr>
      <th>Frecuencia</th>
      <th>Importancia</th>
...
* **Paso 1**: Exploración Abierta (Open)Esta fase inicial consistió en una sesión de "lluvia de ideas" para identificar todos los Eventos de Dominio (Domain Events) significativos dentro del contexto de gestión de estacionamientos de EasyPark. El equipo mapeó eventos que cubren todo el ciclo de vida de las reservas, la operación del recinto y las notificaciones, incluyendo:   Búsqueda y Reserva: Eventos como Search for parking, Check availability, Reserved space y Booking confirmation sent.   Gestión Operativa: Eventos críticos para el control del recinto, incluyendo Vehicle entry recorded, Occupation updated, Space allocated to zone y Vehicle reassigned.   Automatización de Accesos: Hitos impulsados por hardware, como Scanned Plate y Open Doors.   Manejo de Excepciones y Alertas: Identificación preliminar de situaciones irregulares como Maximum stay time exceeded, Expiration notice sent e Irregularity alert generated.

![Brainstorm 1 Open.jpg](../assets/images/Brainstorm%201%20Open.jpg)

* **Paso 2**: Exploración y Líneas de Tiempo (Explore)En este paso, los eventos desestructurados se organizaron en flujos cronológicos para definir el camino ideal (Happy Path) y las ramas operativas, asociándolos a sus respectivos actores clave: Conductor, Administrador, Sensores IoT y sistemas OCR.   Flujo del Conductor (Driver): La línea de tiempo establece la secuencia de búsqueda (ej. Parking lots searched by location), que conduce a la filtración y comparación, culminando en la verificación de disponibilidad (Availability verified), el espacio reservado (Reserved space) y su respectiva confirmación.   Flujo del Administrador (Administrator): Se ilustran las operaciones de supervisión y control de tiempo. El registro de ingreso (Vehicle entry recorded) dispara actualizaciones de ocupación o reasignaciones, así como el envío de notificaciones de tiempo al conductor, lo cual puede derivar en una alerta de retención (Retention alert). También se mapea la resolución operativa (Active alerts checked y Alert resolved) previa al registro de salida.   Flujo de Sistemas Automatizados: Se incorpora la automatización donde el sensor IoT (IoT sensor) detecta salidas para liberar espacios (Liberated space) y actualizar la ocupación, mientras que el OCR escanea placas (Scanned plate) para abrir las puertas automáticamente (Open Doors) asociado a un espacio reservado.
